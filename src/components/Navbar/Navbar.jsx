...
export default function Navbar() {
  return (
    <header className="sticky top-0 z-50 border-b border-[#F0E4DD] bg-white/95 backdrop-blur-md">
      {/* 
        Restored justify-between and added relative positioning 
        to accurately anchor the absolute centered navigation.
      */}
      <div className="relative mx-auto flex w-full max-w-7xl items-center justify-between px-4 py-2 sm:px-6">
        ...
        {/* 
          CHANGES MADE HERE:
          - Swapped flex-1 for `absolute left-1/2 transform -translate-x-1/2`.
          - This detaches the menu from the flanking elements, centering it perfectly in the middle.
        */}
        <nav className="hidden absolute left-1/2 transform -translate-x-1/2 items-center gap-8 text-sm font-semibold text-[#4D3A2A] md:flex">
          <Link to="/shop" className="transition hover:text-[#7A5A3A]">Shop</Link>
        ...
        {/* Kept z-10 to make sure the clickable actions sit layered over background frames if needed */}
        <div className="z-10 flex items-center gap-3">
          ...
          <button className="inline-flex h-11 w-11 items-center justify-center rounded-full bg-[#F7EFE2] text-[#4D3A2A] transition hover:bg-[#E6D4C3]" aria-label="Cart">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="h-5 w-5">
